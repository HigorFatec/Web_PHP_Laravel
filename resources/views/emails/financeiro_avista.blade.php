...
<body>
    ...
    @if ($financeiro->tipo == 'avista' && $financeiro->adiantamento_fornecedor == 'nao')
    ...
    @if ($financeiro->tem_nota_fiscal == 'sim')
        ...
        <p>ID do Lançamento Bancário (RODOPAR): <b> Aguardando Pagamento Financeiro {{ $financeiro->id_raz }}</b></p><br><br>
    @else
        <p><b><center><h4>Solicitação sem Nota Fiscal</h4></center></b></p><br>
        <p>ID do Lançamento no Contas a Pagar (RODOPAR): Fornecedor: <b> Aguardando Pagamento Financeiro {{$financeiro->fornecedor}}</b> Série: <b>A</b> Documento: <b>{{$financeiro->id}}</b></p><br><br>
    @endif
